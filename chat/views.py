...
def index(request):
    ...
    if request.method == 'POST':
        ...
        if not request.method.POST.get('password'):
            return render(request, 'chat/index.html', {'error': True})
        # Username was not set
        elif not request.method.POST.get('username'):
            return render(request, 'chat/index.html', {'error': True})
        # Everything set, adding to db then sending to a burner room!
            ...
            data = {'room_name':room_name, 'password':room_pass 'created_time': datetime.datetime.utcnow()}
            col.insert(data)
...
def room(request, room_name):
    ...
    if request.method == 'GET':
        ...
        if request.session['room_name'] and request.session['room_name'] == room_name:
            # Deleting variable so he will have to relogin next time
            del request.session['room_name']
            return render(request, 'chat/room.html', {'room_name': room_name})
        else:
            return HttpResponse('Room login page!')
    # User submitted form to login, authenticating him
        ...
        room = col.find_one({'room_name':room_name}, {'_id':0, 'created_at':0})
        if room is None:
...
def gen_room(length=random.randint(5, 7)):
    ...
    db_unique = db['data'].find_one({'room_name':room_name})
